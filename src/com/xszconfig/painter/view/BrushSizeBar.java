...
public class BrushSizeBar extends View {
  ...
  /**
   * Constants for saving/restoring the instance state.
   */
  private static final String STATE_PARENT = "parent";
  private static final String STATE_VALUE = "value";

  /**
   * Constants for identifying orientation.
   */
  ...
  /**
   * Factor used to calculate the size to the position on the bar.
   */
  ...
  /**
   * Factor used to toggle orientation between vertical and horizontal.
   */
   ...
   */
  private void calculateSize(int position) {
    // range check
    position = (position < mBarPointerHaloRadius + 1) ? (mBarPointerHaloRadius + 1) : position;
    position = position > (mBarLength + mBarPointerHaloRadius) ? (mBarLength + mBarPointerHaloRadius) : position;

    mSize = Math.round((position - mBarPointerHaloRadius) * mPosToSizeFactor);
    // range check
